...
class _AddProjectPageState extends State<AddProject> {
  ...
  Future<void> _submitForm() async {
    ...
    List<String> imageLinks = _imageLinks.map((controller) => controller.text)
        .toList();
    String email = Provider
        .of<AuthProviderFunction>(context, listen: false)
        .email ?? "";
    ...
    try {
      ...
      if (docSnapshot.exists) {
        existingProjects = (docSnapshot.data() as Map<String,
            dynamic>?)?['projectDetails'] as Map<String, dynamic>? ?? {};
      }

      int nextIndex = existingProjects.keys.isEmpty
          ? 0
          : existingProjects.keys.map(int.parse).reduce((a, b) =>
      a > b
          ? a
          : b) + 1;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Add Project", style: TextStyle(fontWeight: FontWeight
            .bold)),
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                ..._imageLinks.map((controller) =>
                    _buildTextInput(controller, "Image URL")),
                _buildAddButton("Add More Images", _addImageLink),

                // Description
                Text("Description", style: _sectionTitleStyle),
                _buildTextInput(
                    _descriptionController, "Enter project description",
                    maxLines: 3),
                ...
                Row(
                  children: [
                    Expanded(child: _buildTextInput(
                        _creativeFieldController, "Enter creative field")),
                    IconButton(
                ...
                Wrap(
                  children: _creativeFields
                      .map((field) =>
                      Chip(label: Text(field), backgroundColor: Colors.teal))
                      .toList(),
                ),
